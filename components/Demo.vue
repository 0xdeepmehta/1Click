    ...
    >
      ...
      <div class="flex justify-between">
        ...
        <div class="inline-block lg:space-x-6 sm:space-x-2">
          <input
            disabled="true"
            class="py-2 px-4 rounded-lg text-gray-700 text-sm w-72 placeholder-gray-400 font-semibold"
            placeholder="Your secret access token here."
            type="text"
            name="token"
            value="thisisspecialsecreattokenfordemo:)"
          />
          <button
      ...
      <table class="w-full">
        ...
        <tbody class="">
          <tr v-for="row in row_data" :key="row.id" class="">
            <th class="w-4/12 sm:px-1 lg:px-4">
              <input
                v-model="row.key"
                class="w-full sm:px-1 lg:px-2 py-1 rounded-md"
                type="text"
              />
            </th>
            <th class="w-5/12 sm:px-1 lg:px-4">
              <input
                v-model="row.value"
                class="w-full sm:px-1 lg:px-2 py-1 rounded-md"
                type="text"
      ...
      <div class="flex items-center justify-between mt-8">
        <button v-on:click="addNewRow" class="p-3 focus:outline-none">
          <img src="@/assets/plus.svg" alt="Add more" />
        </button>
        <button
          class="flex items-center justify-around bg-green-600 px-4 py-2 rounded-md"
        >
          <span v-on:click="saveData" class="text-white uppercase px-2"
            >Save</span
          >
