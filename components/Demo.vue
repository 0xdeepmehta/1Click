        ...
        <div class="border-2 border-solid border-gray-700 rounded-lg p-4 py-20 lg:px-32 md:px-12">
            <div class="flex justify-between px-10">
                ...
                <div class="inline-block lg:space-x-6 sm:space-x-2">
                    ...
                    <button v-clipboard="() => root_url+ token + '/'" v-tooltip="{content: 'Copied succefully', show: is_saved, trigger: 'click'}" class="inline-block bg-white uppercase text-gray-700 text-sm font-semibold px-4 py-2 rounded-lg">Copy Json Url</button>
            ...
            <table class="w-full">
                ...
                <tbody class="">
                    <tr v-for="row in row_data" :key="row.id" class="">
                        <th class="w-3/12 sm:px-1 lg:px-4"><input v-model="row.key" class="w-full sm:px-1 lg:px-2 py-1 rounded-md" type="text"></th>
                        <th class="w-6/12 sm:px-1 lg:px-4"><input v-model="row.value"  class="w-full sm:px-1 lg:px-2 py-1 rounded-md" type="text"></th>
                        <th class="w-1/12"><a target="_blank" :href="root_url + token + '/' + row.key" class="focus:outline-none w-full flex items-center justify-center">
                            <svg class="fill-current w-7 h-7 text-gray-500" viewBox="0 0 23 19" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <th class="w-1/12"><button  v-tooltip="{content: 'Copied succefully', show: is_saved, trigger: 'click'}" v-clipboard="() => root_url+ token + '/' + row.key" class="focus:outline-none w-full flex items-center justify-center"><img class="w-7 h-7" src="@/assets/link.svg" alt="ClipBoard Button"></button></th>
...
export default {
    data() {
        ...
        key: '',
        value: '',
        root_url: 'https://oneclickdb.herokuapp.com/',
        is_saved: false,
        post_data: {},
        temp: {},
    ...
    methods: {
        ...
        singelView(id) {
            let redirect_url
            if(!this.is_saved){
                redirect_url = window.location + id
        ...
        saveData() {
            ...
            console.log("Final Data: ", JSON.parse(JSON.stringify(this.temp)))
            this.post_data =  JSON.stringify({token:this.token, payload: this.temp})
            console.log(this.post_data)
            // console.log(JSON.parse(JSON.stringify(this.row_data)))
            fetch("https://oneclickdb.herokuapp.com/", { 
